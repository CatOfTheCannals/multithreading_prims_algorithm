...
void Thread::pintarVecinos(Grafo *g, int nodo){
   ...
   for (int i = 0; i < g->listaDeAdyacencias.at(nodo).size(); i++) {
     if(_mst.noEsta(g->listaDeAdyacencias.at(nodo).at(i).nodoDestino)){
       _mstEjes.push(g->listaDeAdyacencias.at(nodo).at(i));
     }
   }
}
